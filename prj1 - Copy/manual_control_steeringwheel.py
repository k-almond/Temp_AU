...
class CameraManager(object):
    def __init__(self, parent_actor, hud):
        ...
        for idx, item in enumerate(self.sensors):
            ...
            if idx == 1:
                ...
                bp.set_attribute('image_size_x', str(950))
                ...
                bp.set_attribute('image_size_x', str(170))
                bp.set_attribute('image_size_y', str(120))
    ...
    def render(self, display):
        ...
        if self.surfaces[1] is not None:
            dashboard_x = (display.get_width() - self.surfaces[1].get_width()) // 2
            dashboard_y = (display.get_height() - self.surfaces[1].get_height()) // 2
            display.blit(self.surfaces[1], (dashboard_x, dashboard_y))
            # Rear Camera
        if self.surfaces[0] is not None:
            rear_camera_x = dashboard_x + (self.surfaces[1].get_width() - self.surfaces[0].get_width()) // 2
            rear_camera_y = dashboard_y - self.surfaces[0].get_height() - 10
            display.blit(self.surfaces[0], (rear_camera_x, 0))

            # Left Mirror
        if self.surfaces[2] is not None:
            display.blit(self.surfaces[2], (dashboard_x - self.surfaces[2].get_width() - 10, dashboard_y))

            # Right Mirror
        if self.surfaces[3] is not None:
            display.blit(self.surfaces[3], (dashboard_x + self.surfaces[1].get_width() + 10, dashboard_y))


        #display.blit(self.surfaces[1], (0, 0))
        #display.blit(self.surfaces[2], (15, 15))
        #display.blit(self.surfaces[3],(display.get_width() - self.surfaces[3].get_width() - 10, 10))
        #display.blit(self.surfaces[0], ((display.get_width() - self.surfaces[0].get_width()) // 2, display.get_height() - self.surfaces[0].get_height() - 10))
        # if self.surfaces[1] is not None:
        #     display.blit(self.surfaces[1], (0, 0))  # Dashboard Camera
        #
        # if self.surfaces[2] is not None:
        #     display.blit(self.surfaces[2], (10, 10))  # Left Mirror
        #
        # if self.surfaces[3] is not None:
        #     display.blit(self.surfaces[3],
        #                  (display.get_width() - self.surfaces[3].get_width() - 10, 10))  # Right Mirror
        #
        # if self.surfaces[0] is not None:
        #     display.blit(self.surfaces[0], ((display.get_width() - self.surfaces[0].get_width()) // 2,
        #                                     display.get_height() - self.surfaces[0].get_height() - 10))  # Rear Camera
